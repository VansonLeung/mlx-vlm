...
```
    ...
    # MiniCPM-o community MLX exports can still require key remapping despite
    # reporting MLX format metadata.
...
def load(
    ...
    **kwargs,
    ...
    if image_processor is not None and str(getattr(model.config, "model_type", "")).lower() not in {
        "minicpmo",
        "minicpm-o",
        "minicpm_o",
    }:
...
def load_processor(
    ...
    # Prefer local backend-specific processor implementation when available.
    if cfg is not None:
        try:
    ...
    if processor is None and is_minicpmo:
        # MiniCPM-o remote processor may pull torch-only dependencies.
        # For MLX text chat fallback, tokenizer-only is sufficient.
